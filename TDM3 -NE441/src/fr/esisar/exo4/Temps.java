...
public class Temps {
    ...
    private static final int DELAY_BETWEEN_SESSIONS = 15000;  // 15 secondes
    private Map<String, Long> dernierJeu;  // Stocker le dernier temps de jeu pour chaque client

    public Temps() {
        dernierJeu = new HashMap<>();
    }
    ...
    public void execute() throws IOException, InterruptedException {
        ...
        while (true) {
            ...
            String adresseClient = socketConnexion.getInetAddress().getHostAddress();
            long tempsActuel = System.currentTimeMillis();

            // Vérifier si le client a déjà joué récemment
            if (dernierJeu.containsKey(adresseClient) && (tempsActuel - dernierJeu.get(adresseClient) < DELAY_BETWEEN_SESSIONS)) {
                // Si le délai de 15 secondes n'est pas encore passé, refuser temporairement la connexion
                System.out.println("Client " + adresseClient + " doit attendre 15 secondes avant de rejouer.");
                socketConnexion.close();
                continue;
            }

            // Mise à jour du dernier temps de jeu pour ce client
            dernierJeu.put(adresseClient, tempsActuel);
